-- Migration: Rollback to simple working version of get_weekly_dashboard_data
-- The complex optimization with localization was causing nested aggregate errors
...
BEGIN
  ...
  -- Get weekly sessions with exercises (SIMPLE VERSION)
  SELECT json_agg(
    json_build_object(
      'id', wss.id,
...
COMMENT ON FUNCTION get_weekly_dashboard_data IS 'Returns weekly workout dashboard data (simple working version)';
