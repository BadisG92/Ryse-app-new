-- Migration: Optimize get_weekly_dashboard_data to include localized exercise names
-- This eliminates the need for 2 additional queries in Flutter (_localizeWeeklySessionsData)
-- Performance: 2000ms → 300ms
...
BEGIN
  ...
  SELECT COALESCE(
    (SELECT language FROM users WHERE id = target_user_id),
    'en'
  ) INTO user_lang;
  ...
  -- Get weekly sessions with exercises (with localized names)
  SELECT json_agg(
    json_build_object(
      'id', wss.id,
      ...
      'exercises', (
        SELECT json_agg(
          json_build_object(
            ...
            'sets_count', COUNT(*),
            -- ⚡ OPTIMISATION: Ajouter le nom localisé directement ici
            'localized_exercise_name', COALESCE(
              -- Si exercice custom, utiliser le nom custom
              (SELECT name FROM custom_exercises ce WHERE ce.id = wsh.custom_exercise_id),
              -- Sinon, utiliser le nom localisé depuis exercises
              CASE
                WHEN user_lang = 'fr' THEN (SELECT name_fr FROM exercises e WHERE e.id = wsh.exercise_id)
                ELSE (SELECT name_en FROM exercises e WHERE e.id = wsh.exercise_id)
              END,
...
-- Add comment
COMMENT ON FUNCTION get_weekly_dashboard_data IS 'Returns weekly workout dashboard data with localized exercise names (optimized - no additional queries needed in Flutter)';
