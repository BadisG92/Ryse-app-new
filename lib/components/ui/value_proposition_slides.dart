...
class _ValuePropositionSlidesState extends State<ValuePropositionSlides>
  ...
  void initState() {
    ...
    // Désactiver toutes les animations internes - uniquement l'animation de slide entre les pages
    _fadeController = AnimationController(
      vsync: this,
      duration: Duration.zero, // Pas d'animation
    );
    _fadeAnimation = const AlwaysStoppedAnimation(1.0); // Toujours visible

    // Stagger animations Slide 1 - désactivées
    _staggerController = AnimationController(
      vsync: this,
      duration: Duration.zero,
    );

    // Stagger animations Slide 2 - désactivées
    _slide2StaggerController = AnimationController(
      vsync: this,
      duration: Duration.zero,
    );
    _slide2CardsOpacity = const AlwaysStoppedAnimation(1.0);

    // Stagger animations Slide 3 - désactivées
    _slide3StaggerController = AnimationController(
      vsync: this,
      duration: Duration.zero,
    );

    // Pas de mouvement - tout visible dès le départ
    _pandaSlide = const AlwaysStoppedAnimation(Offset.zero);
    _pandaOpacity = const AlwaysStoppedAnimation(1.0);
    _titleSlide = const AlwaysStoppedAnimation(Offset.zero);
    _titleOpacity = const AlwaysStoppedAnimation(1.0);
    _cardsSlide = const AlwaysStoppedAnimation(Offset.zero);
    _cardsOpacity = const AlwaysStoppedAnimation(1.0);
  }
  ...
  void _onPageChanged(int page) {
    ...
    // Animation automatique pour slide 2 (anciennement slide 1)
    if (page == 2) {
    ...
    // Animation automatique pour slide 3 (anciennement slide 2)
    if (page == 3) {
  ...
  Widget _buildFooter(bool isFrench, bool isGerman) {
    return Padding(
      ...
      child: Column(
        children: [
          // Dots de pagination (4 slides maintenant)
          Row(
          ...
          Row(
            ...
            children: [
              // Bouton Retour (masqué sur la page welcome)
              if (_currentPage > 0)
                TextButton.icon(
                ...
                Expanded(
                  child: Padding(
                    ...
                    child: TweenAnimationBuilder<double>(
                      duration: Duration.zero, // Pas d'animation
                      tween: Tween(begin: 1.0, end: 1.0), // Toujours visible
                      curve: Curves.linear,
                      builder: (context, value, child) {
                        return child!; // Pas de transformation
                      },
